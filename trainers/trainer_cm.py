...
class Trainer_cm:
    ...
    def _set_signature_columns_if_needed(self):
        if self._signature_columns is None:
            ...
            # 手动添加额外字段
            self._signature_columns += ["cs_input_ids", "cs_codeskeleton_id", "cs_labels"]
    ...
    def ipex_optimize_model(self, model, training=False, dtype=torch.float32):
        ...
        import Trainer.intel_extension_for_pytorch as ipex
    ...
    def _inner_training_loop(
        ...
        tr_cs_loss = torch.tensor(0.0).to(self.args.device)
        tr_acc = torch.tensor(0.0).to(self.args.device)
        # _total_loss_scalar is updated everytime .item() has to be called on tr_loss and stores the sum of all losses
        self._total_loss_scalar = 0.0
        self._total_cs_loss_scalar = 0.0 
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step, tr_cs_loss_step, tr_acc_step = self.training_step(model, inputs, step)
                ...
                if (
                    args.logging_nan_inf_filter
                    and not is_torch_xla_available()
                    and (torch.isnan(tr_cs_loss_step) or torch.isinf(tr_cs_loss_step))
                ):
                    # if loss is nan or inf simply add the average of previous logged losses
                    tr_cs_loss += tr_cs_loss / (1 + self.state.global_step - self._globalstep_last_logged)
                else:
                    if tr_cs_loss.device != tr_cs_loss_step.device:
                        tr_cs_loss_step = tr_cs_loss_step.to(tr_cs_loss.device)
                        raise ValueError(
                            f"Calculated loss must be on the original device: {tr_cs_loss.device} but device in use is {tr_cs_loss_step.device}"
                        )
                    tr_cs_loss += tr_cs_loss_step
                ...
                if (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, tr_cs_loss, tr_acc, grad_norm, model, trial, epoch, ignore_keys_for_eval)
                else:
        ...
        self._total_cs_loss_scalar += tr_cs_loss.item() 
        self._total_acc_scalar += tr_acc.item() 

        effective_global_step = max(self.state.global_step, 0.001)  # Avoid ZeroDivisionError
        train_loss = self._total_loss_scalar / effective_global_step
        train_cs_loss = self._total_cs_loss_scalar / effective_global_step
        train_acc = self._total_acc_scalar / effective_global_step
        ...
        metrics["train_cs_loss"] = train_cs_loss
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_cs_loss, tr_acc, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            tr_cs_loss_scalar = self._nested_gather(tr_cs_loss).mean().item()
            tr_acc_scalar = self._nested_gather(tr_acc).mean().item()

            # reset tr_loss to zero
            tr_loss -= tr_loss
            tr_cs_loss -= tr_cs_loss
            tr_acc -= tr_acc

            logs["loss"] = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["cs_loss"] = round(tr_cs_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            ...
            self._total_cs_loss_scalar += tr_cs_loss_scalar
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], step) -> torch.Tensor:
        ...
        acc = torch.tensor(0.0).to(self.args.device)
        cs_loss = torch.tensor(0.0).to(self.args.device)
        with self.compute_loss_context_manager():
            loss, cs_loss = self.compute_loss(model, inputs, step)
        ...
        if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
            ...
            cs_loss = cs_loss / self.args.gradient_accumulation_steps
            acc = acc / self.args.gradient_accumulation_steps
        
        all_loss = loss + cs_loss  #+ self.cs_loss_alpha * pos_loss 
        if self.use_apex:
            with amp.scale_loss(all_loss, self.optimizer) as scaled_loss:
                scaled_loss.backward()
        else:
            self.accelerator.backward(all_loss, **kwargs)

        return all_loss.detach(), cs_loss.detach(), acc.detach() 
    ...
    def compute_loss(self, model, inputs, step, return_outputs=False):
        """
        Compute custom loss with skeleton mask weighting.
        """
        ...
        if self.label_smoother is not None and "skeleton_mask" in inputs:
            skeleton_mask = inputs.pop("skeleton_mask")
        else:
            skeleton_mask = None
        outputs = None
        cs_loss = torch.tensor(0.0).to(self.args.device) 
        ...
        if step % 2 == 0:
            ...
            curr_inputs = {'input_ids': inputs['cs_input_ids'],'codeskeleton_id': inputs['cs_codeskeleton_id'], 'labels': inputs['cs_labels']}
            cs_loss = model(**curr_inputs)
        ...
        if outputs is not None: 
            ...
            if skeleton_mask is not None:
                ...
                if model_name in MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.values():
                    ...
                    mask_loss = self.label_smoother(outputs, skeleton_mask, shift_labels=True)
                else:
                    mask_loss = self.label_smoother(outputs, skeleton_mask)
            else:
                ...
                mask_loss = outputs["loss"] if isinstance(outputs, dict) else outputs[0]
            final_loss = base_loss + mask_loss
        else:
            final_loss = torch.tensor(0.0).to(self.args.device) 
        
        
        return final_loss, cs_loss
