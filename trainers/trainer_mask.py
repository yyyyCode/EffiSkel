...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        tr_cs_loss = torch.tensor(0.0).to(args.device)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if skeleton_mask is not None:
            ...
            if model_name in MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.values():
                ...
                cs_loss = self.label_smoother(outputs, skeleton_mask, shift_labels=True)
            else:
                cs_loss = self.label_smoother(outputs, skeleton_mask)
        else:
            ...
            cs_loss = outputs["loss"] if isinstance(outputs, dict) else outputs[0]

        final_loss = base_loss + cs_loss
